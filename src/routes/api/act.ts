    ...
    async (req: Request, res: Response): Promise<any> => {
        try {
            ...
            if (!token || !token.serial_number || !token.pid || !token.access_key) {
                console.warn(`/createAccount Access Key/Serial Number/PID doesnt exist for : ${token.pid} ${token.serial_number}`);
                res.status(500).json(
                    {
                        status: "error",
            ...
            if (formPid != headerPid) {
                console.warn(`Unintended Behavior (PID Mismatch): ${formPid}, ${headerPid}`);
                res.status(500).json(
                    {
                        status: "error",
            ...
            if (existing) {
                console.warn(`Account already exists for: ${token.pid} set up from Serial Number ${token.serial_number}`);
                res.status(500).json(
                    {
                        status: "error",
            ...
            const checkPID = await fetch(`https://mii-unsecure.ariankordi.net/mii_data/?pid=${headerPid}&api_id=1`);
            if (!checkPID.ok) {
                console.warn(`Mii Unsecure Pretendo fetching error for : ${token.pid} ${token.serial_number}`);
                res.status(500).json(
                    {
            ...
            if (data.country && country.toUpperCase() !== data.country.toUpperCase()) {
                res.status(500).json({
                    status: "error",
            ...
            let lang = rawLang.includes(',') || rawLang.includes('-')
            ...
            const xOauthToken = data.xOauthToken === '' ? null : data.xOauthToken;
            const xOauthSecret = data.xOauthSecret === '' ? null : data.xOauthSecret;
            const xUserId = data.xUserId === '' ? null : data.xUserId;

            //Used for creating the session, not actually stored on the DB.
            ...
            if (bskyUsernameTemp.length >= 1 && bskyPasswordTemp.length >= 1) {
                ...
                bskySession = await bsky.login(bskyUsernameTemp, bskyPasswordTemp);

                if (!bskySession) {
                    console.warn(`Error fetching BlueSky data for: ${token.pid} ${token.serial_number} ${bskySession}`);
                }
            } else {
                bskySession = null;
            }
            ...
            if (ipInfo && ipInfo.success && ipInfo.timezone && typeof ipInfo.timezone.offset === "number") {
                utc_offset = ipInfo.timezone.offset;
            } else {
                res.status(500).json({ status: "error", error: "Could not get the users UTC offset." });
            }
            ...
            if (!result) {
                console.warn(`Invalid data submitted for : ${token.pid} ${token.serial_number} ${result}`);
                res.status(500).json(
                    {
                        status: "error",
            ...
            if (!userId) {
                console.warn(`Failed to insert account to DB : ${token.pid} ${token.serial_number}`);
                res.status(500).json(
                    {
                        status: "error",
            ...
            const createdSettings = await db('settings').insert({
                ...
                x_user_id: xUserId,
                bsky_auth_session_json: bskySession
            });

            if (!Array.isArray(createdSettings) || createdSettings.length === 0) {
                console.warn(`Failed to submit settings information on database : ${token.pid} ${token.serial_number}`);
                res.status(500).json({ error: "Account creation failed, account created but settings could not be inserted." });
                return;
            }
...
router.post(
    ...
    async (req: Request, res: Response): Promise<any> => {
        try {
            ...
            if (!token || !token.access_key || !token.serial_number || !token.pid) {
                console.warn(`/checkLogIn Access Key/Serial Number/PID doesn't exist for: ${token.pid} ${token.serial_number}`);
            ...
            const rawLang = (req.headers['accept-language'] ?? '') as string;
            const lang = rawLang.includes(',') || rawLang.includes('-')
                ? rawLang.split(',')[0].split('-')[0].toLowerCase()
                : rawLang.toLowerCase();
            ...
            if (now.getTime() - lastUpdate.getTime() > oneHour) {
                const updateMiiData = await fetch(`https://mii-unsecure.ariankordi.net/mii_data/?pid=${token.pid}&api_id=1`);
                ...
                console.log(`PNID Data and UTC offset updated for PID: ${token.pid}`);
            }
            ...
            if (!userSettings) {
                return res.status(400).json({ status: "error", error: "Could not get user settings." });
            }
