...
router.get("/countries/CA", async (req: Request, res: Response): Promise<any> => {
    const { type, region, city } = req.query as { type?: string, region?: string, city?: string };
    ...
    try {
        ...
        if (type === "regions") {
            const items: Array<{ state: string; city: string }> = payload.data.items;
            ...
            const filteredProviders = payload.data.items.filter((item: any) => {
...
router.get("/lineup/:providerId", async (req: Request, res: Response): Promise<any> => {
    const { providerId } = req.params;
    let { start, duration = "120", offset = "0", limit = "100"} = req.query as {
        start?: string;
        duration?: string;
        offset?: string;
        limit?: string;
    ...
    if (!durationResult.success || !startResult.success || !offsetResult.success) {
        return res.status(400).json({
            endpoint,
            hasError: 1,
    ...
    if (cachedData) {
        ...
        try {
            ...
            transformedItems = data.data.items.map((item: LineupItem) => ({
                ...
                programSchedules: item.programSchedules.map((program: ProgramSchedule) => ({
                    ...program,
    ...
    // Slice the data
    const paginatedItems = transformedItems.slice(validatedOffset, validatedOffset + numericLimit);
...
router.get("/program/:programId/details", async (req: Request, res: Response) => {
    ...
    try {
        ...
        logger.error(`Error in ${endpoint}/${validatedProgramId}?type=${typeString}: ${e}`);
